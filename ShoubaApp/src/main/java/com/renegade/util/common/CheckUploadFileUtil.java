...
public class CheckUploadFileUtil {  
	...
	//允许上传的文件类型
	public static final String[] imgType = {".jpg",".png"};
...
//      System.out.println(builder.toString());  
     ...
     * @return
     */
    public static AjaxJson checkFileIllegal(MultipartFile file) {
    	boolean flag=false;
    	AjaxJson ajaxJson=new AjaxJson();
    	//文件不能为空
        if(file.isEmpty()) {
        	ajaxJson.setSuccess(false);
    		ajaxJson.setMsg("文件不能为空");
    		return ajaxJson;
        }
    	//文件类型校验
        if (StringUtils.isNotBlank(file.getContentType())) {
        	...
        	try {
        		...
        		ajaxJson.setSuccess(false);
        		ajaxJson.setMsg("文件头校验异常");
        		return ajaxJson;
        	}
        	String fileType=mFileTypes.get(type);
        	for (int i=0;i<imgType.length;i++) {
        		if(imgType[i].equals(fileType)) {
        			flag=true;
        		}
        	}
        	if(flag==false) {
        		ajaxJson.setSuccess(false);
        		ajaxJson.setMsg("文件类型异常");
        		return ajaxJson;
        	}
        }
