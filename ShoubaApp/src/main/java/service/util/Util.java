...
public class Util {
	...
	/**
	 * 
	 * ???????????byte??????98ABC???????00001001 00001000 00001010 00001011 00001100
	 * ??????????0-9 a-f A-F??22???????????????????????????????
	 */
	...
	/**
	 * 
	 * bcd str?bytes,????86AE???????10000110 10011110
	 * ??????????0-9 a-f A-F??22???????????BCD???????????????????????
	 */
	...
	/**
	 * 
	 * bytes?BCD????????01001100 00011000(bytes[0]=01001100,bytes[1]=00011000)???????4C18
	 */
	...
	/**
	 * 
	 *????????????????????00010000 00000001(bytes[0]=00010000,bytes[1]=00000001)???????0001000000000001
	 */
	...
	/**
	 * 
	 * 10???????BCD??(???????)??????0123456789???????00000001 00100011 01000101 01100111 10001001
	 * ??????????0-9 a-f A-F??22?????
	 * ??????????0?????????????F??
	 */
	...
	/**
	 * 
	 * ??????????????(???????)??????00110100 01110001 11000001(24?????)???????00110100 01110001 11000001(3???)
	 * ????????????????0??1
	 */
	public static byte[] binStr2Bin(String str) {
		...
		if(temp.length%8 != 0) throw new RuntimeException("?????'"+str+"'???????8???????????????????");
		for(int i= 0;i < temp.length;i++) {
			if((temp[i]-'0') < 0 || (temp[i]-'0') > 1) throw new RuntimeException("?????'"+str+"'????'0'??'1'??????????????????????");
		}
	...
	/**
	 * ????1001(4bits)???????00000001 00000000 00000000 00000001(4bytes)
	 */
	...
	/**
	 * ????00000001 00000000 00000000 00000001(4bytes)???????1001(4bits)
	 */
	...
	/**
	 * ????95BE5779 04DC9CF7???????39 35 42 45 35 37 37 39  30 34 44 43 39 43 46 37
	 */
	...
	/**
	 * ????00000000 00111100 (0x00,0x3C)  ?????60
	 */
